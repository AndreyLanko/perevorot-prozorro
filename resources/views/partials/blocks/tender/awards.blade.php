    ...
    <div class="container wide-table">
        <div class="tender--offers margin-bottom-xl">
            ...
            @if (1==1 || !empty($item->__qualifications))
                <div style="margin-top:-10px;margin-bottom:40px">Друкувати протокол розкриття тендерних пропозицій <a href="{{href('tender/'.$item->tenderID.'/print/awards/'.$item->id.'/pdf')}}" target="_blank">PDF</a> ● <a href="{{href('tender/'.$item->tenderID.'/print/awards/'.$item->id.'/html')}}" target="_blank">HTML</a></div>
            @endif
